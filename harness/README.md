...
- `run_eval.py` — drives `gemini-1.5-flash` (temperature 0) against the **session-free** Jaeger MCP endpoint `http://localhost:16686/api/ai/mcp/`. All model calls go through `llm.call_llm(prompt, tools)`.
