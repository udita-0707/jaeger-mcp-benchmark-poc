...
**This POC covers:** 1 valid fault scenario (`cartFailure`), 1 documented-broken scenario (`productCatalogFailure`, not scored), 1 LLM (`gemini-1.5-flash`, temperature 0), baseline trajectory capture against the live session-free MCP server, manual metric computation via `score.py`.
